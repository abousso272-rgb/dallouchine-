...
export const GroupageCard: React.FC<GroupageCardProps> = ({ groupage }) => {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 hover:border-[#FF4500]/50 shadow-xs hover:shadow-xl transition-all duration-300 flex flex-col overflow-hidden group">
      ...
      >
        ...
        <div className="absolute top-3 left-3 flex items-center gap-2">
          <span className="bg-[#0B192C] text-white font-extrabold text-xs px-2.5 py-1 rounded-lg shadow-sm flex items-center gap-1.5">
        ...
        <div className="absolute bottom-3 left-3 bg-slate-900/80 backdrop-blur-md text-white text-[11px] font-medium px-2.5 py-1 rounded-lg flex items-center gap-1.5">
          {groupage.transportMode === 'air' ? (
            <>
              <Plane className="w-3.5 h-3.5 text-orange-300" />
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div>
          <h3
            onClick={() => navigate(`/groupages/${groupage.id}`)}
            className="text-sm sm:text-base font-bold text-slate-900 line-clamp-2 hover:text-[#FF4500] cursor-pointer transition-colors leading-snug"
          >
            {groupage.title}
          </h3>
          ...
          <div className="mt-3 flex items-baseline gap-2.5">
            <span className="text-xl sm:text-2xl font-black text-[#0B192C] tracking-tight">
          ...
          <div className="mt-4 pt-2 border-t border-slate-100">
            <ProgressBar
              ...
              unitLabel="unités"
              colorScheme={groupage.status === 'closing_soon' ? 'amber' : 'orange'}
            />
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-[11px] bg-slate-50 p-2.5 rounded-xl border border-slate-100">
            <div className="flex items-center gap-1.5 text-slate-600">
              <Calendar className="w-3.5 h-3.5 text-[#FF4500] shrink-0" />
              <div>
        ...
        <div className="pt-2 flex items-center gap-2">
          <button
            onClick={() => navigate(`/groupages/${groupage.id}`)}
            className="flex-1 bg-[#FF4500] hover:bg-[#E03D00] text-white font-bold text-xs py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 shadow-xs transition-all active:scale-95"
          >
